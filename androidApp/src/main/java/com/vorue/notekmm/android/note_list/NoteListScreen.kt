...
fun NoteListScreen(navController: NavController , viewModel: NoteListViewModel = hiltViewModel()) {
    ...
    Scaffold(floatingActionButton = {
        FloatingActionButton(onClick = { navController.navigate("note_detail/-1L") }, backgroundColor = Color.Black) {
            Icon(
                imageVector = Icons.Default.Add, tint = Color.White, contentDescription = "Add Note"
            )
        }
    }) { paddingValues ->
...
fun PreviewNoteListScreen() {
    NoteListScreen(navController = NavController(LocalContext.current))
}